...
/**
 * Base class that represents a row from the 'quizes' table.
 *
 * 
 *
 * @package    Bj.om
...
abstract class BaseQuiz extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array QuizQuestion[] Collection to store aggregation of QuizQuestion objects.
	 */
	protected $collQuizQuestions;

	/**
	 * @var        Criteria The criteria used to select the current contents of collQuizQuestions.
	 */
	private $lastQuizQuestionCriteria = null;
	...
	public function setId($v)
	{
		...
		if ($this->quiz_id !== $v) {
			$this->quiz_id = $v;
			$this->modifiedColumns[] = QuizPeer::QUIZ_ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = QuizPeer::NAME;
		}
	...
	public function setSlug($v)
	{
		...
		if ($this->slug !== $v) {
			$this->slug = $v;
			$this->modifiedColumns[] = QuizPeer::SLUG;
		}
	...
	public function setDeletedAt($v)
	{
		...
		if ( $this->deleted_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = QuizPeer::DELETED_AT;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 7; // 7 = QuizPeer::NUM_COLUMNS - QuizPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->lastQuizQuestionCriteria = null;
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if (!empty($ret) && QuizPeer::isSoftDeleteEnabled()) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(QuizPeer::QUIZ_ID)) $criteria->add(QuizPeer::QUIZ_ID, $this->quiz_id);
		if ($this->isColumnModified(QuizPeer::NAME)) $criteria->add(QuizPeer::NAME, $this->name);
		if ($this->isColumnModified(QuizPeer::SLUG)) $criteria->add(QuizPeer::SLUG, $this->slug);
		...
		if ($this->isColumnModified(QuizPeer::DELETED_AT)) $criteria->add(QuizPeer::DELETED_AT, $this->deleted_at);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(QuizPeer::QUIZ_ID, $this->quiz_id);
	...
	/**
	 * Clears out the collQuizQuestions collection (array).
	 *
	...
	/**
	 * Initializes the collQuizQuestions collection (array).
	 *
	 ...
	 * @return     void
	 */
	public function initQuizQuestions()
	{
		$this->collQuizQuestions = array();
	}
	 ...
	 * related QuizQuestions from storage. If this Quiz is new, it will return
		...
		if ($this->collQuizQuestions === null) {
			if ($this->isNew()) {
			   $this->collQuizQuestions = array();
				...
				$this->collQuizQuestions = QuizQuestionPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				QuizQuestionPeer::addSelectColumns($criteria);
				if (!isset($this->lastQuizQuestionCriteria) || !$this->lastQuizQuestionCriteria->equals($criteria)) {
					$this->collQuizQuestions = QuizQuestionPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastQuizQuestionCriteria = $criteria;
	...
	public function countQuizQuestions(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collQuizQuestions === null) {
			if ($this->isNew()) {
				...
				$count = QuizQuestionPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastQuizQuestionCriteria) || !$this->lastQuizQuestionCriteria->equals($criteria)) {
					...
					$count = count($this->collQuizQuestions);
				}
			} else {
				$count = count($this->collQuizQuestions);
			}
	...
	public function addQuizQuestion(QuizQuestion $l)
	{
		...
		if (!in_array($l, $this->collQuizQuestions, true)) { // only add it if the **same** object is not already associated
			array_push($this->collQuizQuestions, $l);
			$l->setQuiz($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collQuizQuestions) {
				foreach ((array) $this->collQuizQuestions as $o) {
